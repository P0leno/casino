...
def is_user_banned(user_id: int) -> bool:
    """Проверка бана пользователя в БД (MAIN DB)"""
...
def ban_user_in_support(user_id: int, until_date: str = None):
    """Забанить пользователя ТОЛЬКО в поддержке (SUPPORT DB)
    ...
    if not user_exists:
        # Создаем пользователя если его нет (только support поля)
        print(f"Creating user {user_id} in support DB...")
        cursor.execute(
    ...
    # Обновляем support_banned и support_banned_until
    cursor.execute(
...
def unban_user_in_support(user_id: int):
    """Разбанить пользователя ТОЛЬКО в поддержке (SUPPORT DB)"""
    conn = get_support_db_connection()
    cursor = conn.cursor()
...
def is_user_banned_in_support(user_id: int) -> bool:
    """Проверка бана в поддержке с учётом временного бана (SUPPORT DB)"""
    ...
    if banned_until:
        try:
            ...
            if now >= until_dt:
                cursor = get_support_db_connection().cursor()
                cursor.execute(
...
def unban_user(user_id: int):
    """Разбанить пользователя в БД (MAIN DB)"""
    conn = get_db_connection()
    cursor = conn.cursor()
...
def get_active_dialog(user_id: int):
    """Получить активный диалог пользователя (SUPPORT DB)"""
    conn = get_support_db_connection()
    cursor = conn.cursor()
    cursor.execute(
...
def create_dialog(user_id: int, username: str, category: str):
    """Создать новый диалог (SUPPORT DB)"""
    conn = get_support_db_connection()
    cursor = conn.cursor()
    cursor.execute(
...
def update_last_response(dialog_id: int):
    """Обновить время последнего ответа (SUPPORT DB)"""
    conn = get_support_db_connection()
    cursor = conn.cursor()
    cursor.execute(
...
def save_message_to_dialog(dialog_id: int, sender_type: str, sender_name: str, message_text: str = None, photo_path: str = None):
    """Сохранить сообщение в историю диалога (SUPPORT DB)"""
    conn = get_support_db_connection()
    cursor = conn.cursor()
    cursor.execute(
...
def delete_dialog_photos(dialog_id: int):
    """Удалить все фото диалога из app/temp (SUPPORT DB)"""
    try:
        conn = get_support_db_connection()
        cursor = conn.cursor()
        cursor.execute(
...
async def generate_dialog_html(dialog_id: int) -> str:
    """Генерировать HTML файл диалога и вернуть путь к нему (SUPPORT DB)"""
    import pytz
...
def get_close_keyboard(dialog_id: int):
    """Клавиатура с кнопкой закрытия диалога и приоритетом (SUPPORT DB)"""
...
async def handle_miniapp_unban_user(callback: CallbackQuery):
    ...
    try:
        ...
        # Разбаниваем в БД (MAIN APP) - используем unban_user который использует Main DB
        ...
        # Получаем категорию диалога (SUPPORT DB)
        conn = get_support_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT category FROM support_dialogs WHERE dialog_id = ?", (dialog_id,))
        result = cursor.fetchone()
...
async def handle_reject_appeal(callback: CallbackQuery):
    ...
    try:
        ...
        # Выдаём бан в поддержке на 1 день (SUPPORT DB) - ban_user_in_support уже использует Support DB
        from datetime import datetime, timedelta
...
async def handle_unban_user(callback: CallbackQuery):
    ...
    try:
        ...
        # Разбаниваем ТОЛЬКО в поддержке (SUPPORT DB) - unban_user_in_support уже использует Support DB
...
async def handle_withdraw_done(callback: CallbackQuery):
    ...
    try:
        ...
        # Получаем user_id из диалога (SUPPORT DB)
        conn = get_support_db_connection()
        cursor = conn.cursor()
...
async def handle_user_data(callback: CallbackQuery):
    ...
    try:
        ...
        # Получаем данные пользователя из БД (SUPPORT DB)
        conn = get_support_db_connection()
        cursor = conn.cursor()
        cursor.execute(
